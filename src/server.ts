...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname === "/auth/start") {
    try {
      ...
      const { clientId } = await registerClient(redirectUri, "Kravr");
...
<title>Kravr — Admin Dashboard</title>
...
function exportLogs() {
  fetch('/api/logs?limit=500').then(function(r){return r.json()}).then(function(data) {
    ...
    data.integration = 'Kravr — Builders Club';
...
server.listen(PORT, () => {
  ...
  console.log(`\n  Kravr server running at http://localhost:${PORT}`);
