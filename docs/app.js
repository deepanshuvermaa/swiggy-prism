...
function showConnectOverlay() {
  ...
  overlay.innerHTML = '<div style="background:white;border-radius:20px;padding:28px 24px;max-width:340px;width:100%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.2);animation:successPop 0.4s ease">'
    ...
    + '<p style="font-size:13px;color:#666;line-height:1.6;margin-bottom:20px">Connecting to Swiggy requires the Kravr backend server running locally. This demo runs on GitHub Pages (static).</p>'
...
function renderXRayFromServer(data) {
  ...
  if (insight && foodOrders.length > 0) {
    ...
    insight.innerHTML = '<h4>Real Spending Insight</h4><p>Your average Swiggy Food order is ₹' + avgFood +
      ...
      '<span class="savings-amount">Kravr can save you 30-40% by switching some orders to cooking.</span></p>';
  }
}
...
// ─── F4: Social Sharing / Kravr Wrapped ───────────────────────────────────────
...
function shareWrapped() {
  ...
  ctx.fillText('Powered by Kravr · ' + new Date().toLocaleDateString('en-IN', { month: 'long', year: 'numeric' }), 200, 560);

  // Show share modal
  var preview = document.getElementById('share-preview');
  if (preview) preview.innerHTML = '<img src="' + canvas.toDataURL('image/png') + '" alt="Kravr Wrapped">';
...
function shareToWhatsApp() {
  var text = 'Check out my Kravr Wrapped! I saved ' + getWrappedStats().savingsPct + '% on food this month 🔥';
  if (navigator.share) {
    var canvas = document.getElementById('wrapped-canvas');
